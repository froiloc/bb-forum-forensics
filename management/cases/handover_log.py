...
def build_handovers(assign_events: List[dict], names: Dict[int, str],
    ...
    {user_id, seq, ts, actor_id, assigned_to} — muss NICHT vorsortiert sein;
    ...
    for ev in assign_events:
        by_case.setdefault(int(ev["user_id"]), []).append(ev)
    ...
    for uid, lst in by_case.items():
        ...
        for ev in lst:
            ...
            entries.append(HandoverEntry(
                user_id=uid, seq=int(ev["seq"]), ts=int(ev["ts"]), kind=kind,
...
def handover_to_dict(report: HandoverReport) -> dict:
    return {
        ...
        "entries": [
            {"user_id": e.user_id, "seq": e.seq, "ts": e.ts, "kind": e.kind,
