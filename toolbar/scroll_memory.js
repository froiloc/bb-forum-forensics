 ...
 * ---------------------------------------------------------------------------
 * Version: v0.8.473 · Build: 473 · 2026-07-21
 *
 * ZWECK
 ...
 *   Build 473: Anker-primaer + Y-Fallback (dieser Build). Persistenzschema
 *              additiv um {anchor, offset} erweitert — 471er-Datensaetze ohne
 *              Anker bleiben lesbar und laufen ueber den Y-Pfad.
 *
...
(function () {
  ...
  function hasRestore(entry, minPx) {
    ...
    if (entry.anchor && isStableId(entry.anchor)) return true;
   ...
   * @returns {{y:number, via:('anchor'|'y'|'none')}}
   */
  function resolveTarget(entry, anchorAbsTopNow, curDpr, scrollHeight, innerHeight, minPx) {
    if (entry && entry.anchor && isStableId(entry.anchor) && typeof anchorAbsTopNow === 'number') {
      var raw = anchorTarget(anchorAbsTopNow, entry.offset);
      return { y: clampTarget(raw, scrollHeight, innerHeight), via: 'anchor' };
    }
  ...
  ScrollMemory.prototype.attach = function () {
    ...
    this._dbg('attach OK (Build 473, Anker-primaer). gespeicherte Seiten=' +
