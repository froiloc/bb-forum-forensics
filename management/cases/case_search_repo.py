...
#   Kommandopalette. Numerische Eingabe -> subject_id-Treffer (plus username-
...
class CaseSearchRepo:
    ...
    def search(self, *, q: str, scope: str = "alle",
        ...
        # Numerische Eingabe: subject_id ODER username-Teilstring.
        if term.isdigit():
            where.append("(c.subject_id = ? OR c.username LIKE ? ESCAPE '\\')")
        ...
        sql = (
            "SELECT c.subject_id, c.username, c.status, i.system_username "
            "FROM cases c LEFT JOIN person i ON i.id = c.assigned_to "
            "WHERE " + " AND ".join(where) +
            # Exakte subject_id-Treffer zuerst (bei numerischer Suche), dann uid.
            " ORDER BY (CASE WHEN c.subject_id = ? THEN 0 ELSE 1 END), c.subject_id "
            "LIMIT ?"
        )
        ...
        rows = rows[:limit]
        results = [
            {"subject_id": int(r[0]), "username": r[1], "status": r[2],
