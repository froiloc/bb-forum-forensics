#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# bundle_einspielen.sh -- Einspielen einer Bundle-Lieferung (data-exchange.md)
#
...
package="${1:-}"
build_no="${2:-}"
testbefehl="${3:-python run_tests.py}"
...
aufloesen() {
    if ! git config --get merge.tool >/dev/null 2>&1; then
        ...
        echo "Einrichten z.B.:  git config --global merge.tool vimdiff"
...
if [ -n "$(git status --porcelain)" ]; then
    echo "Schmutziger Arbeitsbaum auf master."
    ...
    git status --short
    # OHNE -u: unverfolgte Dateien bleiben liegen. Sie sind durch den Merge
    # nicht gefaehrdet -- kollidiert eine, bricht git laut ab. Sie mitzunehmen
    # vergroessert nur die Angriffsflaeche (siehe Befund zur Bundle-Datei).
    git stash push -m "vor ${package} Build ${build_no}"
    stash_kennung="$(git rev-parse 'stash@{0}')"
